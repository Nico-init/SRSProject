...
Unzip the zip folder and put the 'geckodriver' file in /usr/bin directory
'''
...
'''
def get_user_stats(users, user_id):
    #get score and starting_point for a specific user
    #and i need an % accumulator and a counter for each user
    #-> the score is the average (accumulator / counter)
    user_stats.total = 0
    user_stats.n = 0
    pass
'''
...
def scrape_value_yahoo(stock):
    ...
    try:
        #Cookie wall
        elem = driver.find_element(by=By.XPATH, value="//button[@name='agree']")
        elem.click()
        #If the redirection doesn't work automatically
        elem = driver.find_element(by=By.XPATH, value="//div[@class='loader-container']//a")
        elem.click()
    except:
        #no cookies wall or no redirection -> it can happen, so ok! (**magic mistery**)
        pass
